function [winner, err, errstr] = battle_v2(robot1, robot2, map_opt, show, speed)
...
if params.game.show
    ...
    % if recording, initialize avi object
    if params.game.record
        aviobj = VideoWriter(sprintf('%s vs %s on %s.avi', robot(1).team, robot(2).team, map.name));
        aviobj.FrameRate = 10;
        open(aviobj)
        % aviobj.quality = 25;
...
for i = 0:params.game.nTurns
    ...
    if params.game.show
        ...
        % if game record is on, record this frame
        if params.game.record
...
for i = 1:2
    for j = 1:length(map.mine)
        if norm(map.robot(i).input.pos - map.mine(j).pos, 2) <= params.game.m_dist
...
for i = 1:length(map.handle.tank)
    if map.handle.tank(i).delete
        ...
        tank_del(i) = 1;
%     else
%         set(map.handle.tank(i).body, 'MarkerFaceColor', rand(1,3))
%         set(map.handle.tank(i).body, 'MarkerEdgeColor', rand(1,3))
    end
end
...
for i = 1:2
    ...
    % set trail (set to params! 1000 reset to 25 because it looks better)
    trail_x = [get(map.handle.robot(i).trail, 'XData'), map.robot(i).input.pos(1)];
    trail_y = [get(map.handle.robot(i).trail, 'YData'), map.robot(i).input.pos(2)];
    tail = 1000;
    trail_x = trail_x(max([length(trail_x) - tail, 1]):end);
    trail_y = trail_y(max([length(trail_y) - tail, 1]):end);
    
    set(map.handle.robot(i).trail,'XData', trail_x);
    set(map.handle.robot(i).trail,'YData', trail_y);
    
    fuel_y = get(map.handle.robot(i).fuelbar, 'YData');
    fuel_y(2) = 2 + 94*map.robot(i).input.fuel/params.game.f_max;
    
    set(map.handle.robot(i).fuelbar, 'Ydata', fuel_y)
end
...
% plot border - divide by 2 added so that top border would show up on
% etcheverry lab computers.
plot([params.game.x_min-params.graphics.border_offset,...
    ...
    params.game.x_min-params.graphics.border_offset],...
    [params.game.y_min-params.graphics.border_offset/2,...
    params.game.y_min-params.graphics.border_offset/2,...
    params.game.y_max + params.graphics.border_offset/2,...
    params.game.y_max + params.graphics.border_offset/2,...
    params.game.y_min-params.graphics.border_offset/2],...
    'Color', 'k',...
...
for i = 1:2
    ...
    % THESE ARE NOT PARAMETERIZED YET!
    % plot trail
    handle.robot(i).trail = plot(map.robot(i).input.pos(1), map.robot(i).input.pos(2),...
        'Color', map.robot(i).color,...
        'LineStyle', '--',...
    'LineWidth', 3);
    ...
    handle.robot(i).fuelbar = plot([x x], [2, 2+96*params.game.f_0/params.game.f_max],'LineWidth',8,'Color',map.robot(i).color);
end
